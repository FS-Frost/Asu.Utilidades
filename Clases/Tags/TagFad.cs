...
namespace Asu.Utilidades.Clases {
    ...
    public class TagFad : Tag {
        public override string Nombre => "fad";
        ...
        public TagFad(string texto) {
            var regex = new Regex(ExpresionesRegulares.regexTagFad);
            var match = regex.Match(texto);
            if (match.Success) {
                var regex2 = new Regex(ExpresionesRegulares.regexArgsFad);
        ...
        public override string ToString() {
            return string.Format("\\{0}({1},{2})", Nombre, T1, T2);
        }
    }
}
