...
namespace Asu.Utilidades.Clases {
    ...
    public class TagT : Tag {
        ...
        public TagT(string texto) {
            var regex = new Regex(ExpresionesRegulares.regexTagT);
            var match = regex.Match(texto);
            if (match.Success) {
                var regex2 = new Regex(ExpresionesRegulares.regexArgsT);
        ...
        public string ToStringAsTimed() {
            return string.Format("\\{0}({1},{2},{3})", Nombre, T1, T2, Argumento);
        }
        ...
        public string ToStringAsAccelerated() {
            return string.Format("\\{0}({1},{2})", Nombre, Accel, Argumento);
        }
        ...
        public string ToStringAsTimedAccelerated() {
            return string.Format("\\{0}({1},{2},{3},{4})", Nombre, T1, T2, Accel, Argumento);
        }
    }
}
